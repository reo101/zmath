...
test "root surface links ga and vga entrypoints" {
    ...
    const e1 = E2.e(1);
    const rotor = ga.rotors2d.planarRotor(f64, std.math.pi / 2.0);
    const turned = ga.rotors2d.rotated(e1, rotor);
    try std.testing.expect(ga.rotors2d.nearlyEqual(turned.coeff(0b01), 0.0, 1e-12));
    try std.testing.expect(ga.rotors2d.nearlyEqual(turned.coeff(0b10), 1.0, 1e-12));
}
