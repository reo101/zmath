...
pub const Hyper = struct {
    pub const naming_options = ga.SignedBladeNamingOptions.withBasisSpans(ga.BasisIndexSpans.init(.{
        .positive = .range(1, 3),
        .negative = .singleton(0),
    }));
    pub const Algebra = ga.AlgebraWithNamingOptions(.{ .p = 3, .q = 1, .r = 0 }, naming_options).Instantiate(f32);
...
pub const Round = struct {
    ...
    pub const Algebra = ga.AlgebraWithNamingOptions(.{ .p = 4, .q = 0, .r = 0 }, naming_options).Instantiate(f32);
    pub const Vector = Algebra.Vector;
